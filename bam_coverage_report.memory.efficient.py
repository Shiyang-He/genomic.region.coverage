...
def parseOptions():
    desc='''This script performs the same job as bam_coverage_report.py, however it save some memory when compute the whole geome or large regions. Instead of storing depth in a big list, I store a dict {coverage => how many base have that coverage}, to compute percentile, etc. In reality, it's not very possible that you have several million depths to store.''' 
    epilog='''usages:
python3 bam_coverage_report.memory.efficient.py -i input.bam -o output -g ACTD -d
python3 bam_coverage_report.memory.efficient.py -i input.bam -o output -b a.bed
python3 bam_coverage_report.memory.efficient.py -i input.bam -o output -r chr:start-end -d
'''
...
def compute_percentle_from_two_arrays(coverage_values, coverage_bases, total_bases):
    p10, p25, p50, p75, p90 = None, None, None, None, None 
    # start accumulate bases and count percentile
    cummulate_base=0
    for i in range(len(coverage_values)):
        cummulate_base += coverage_bases[i]
        if cummulate_base >= total_bases * 0.9:
            if p10 == None: p10 = coverage_values[i]
            if p25 == None: p25 = coverage_values[i]
            if p50 == None: p50 = coverage_values[i]
            if p75 == None: p75 = coverage_values[i]
            if p90 == None: p90 = coverage_values[i] 
        elif cummulate_base >= total_bases * 0.75:
            if p10 == None: p10 = coverage_values[i]
            if p25 == None: p25 = coverage_values[i]
            if p50 == None: p50 = coverage_values[i]
            if p75 == None: p75 = coverage_values[i]
        elif cummulate_base >= total_bases * 0.5:
            if p10 == None: p10 = coverage_values[i]
            if p25 == None: p25 = coverage_values[i]
            if p50 == None: p50 = coverage_values[i]
        elif cummulate_base >= total_bases * 0.25:
            if p10 == None: p10 = coverage_values[i]
            if p25 == None: p25 = coverage_values[i]
        elif cummulate_base >= total_bases * 0.1:
            if p10 == None: p10 = coverage_values[i]
    return p10,p25,p50,p75,p90

def compute_coverage_stats(depths, total_bases):
    '''Compute basic and percentile coverage statistics. store the results in two dicts: key stats numbers and values to plot piechart'''
    # depths is a dict, key is depth, value is how many bases have that depth
    coverage_values = np.array(list(depths.keys()))   # coverage value 
    coverage_bases = np.array(list(depths.values()))  # bases that have certain coverage
    avg_depth = sum(coverage_values * coverage_bases) / total_bases 
    max_depth = max(depths.keys())
    zero_cov = sum(coverage_bases[np.where(coverage_values ==0)[0]]) 
    over_1_less_5 = sum(coverage_bases[np.where((coverage_values >=1) & (coverage_values <5))[0]])
    over_5_less_10 = sum(coverage_bases[np.where((coverage_values >=5) & (coverage_values <10))[0]])
    over_10_less_30 = sum(coverage_bases[np.where((coverage_values >=10) & (coverage_values <30))[0]])
    over_30_less_50 = sum(coverage_bases[np.where((coverage_values >=30) & (coverage_values <50))[0]])
    over_50_less_100 = sum(coverage_bases[np.where((coverage_values >=50) & (coverage_values <100))[0]])
    over_100_less_200 = sum(coverage_bases[np.where((coverage_values >=100) & (coverage_values <200))[0]])
    over_1 = sum(coverage_bases[np.where(coverage_values >=1)[0]])
    over_5 = sum(coverage_bases[np.where(coverage_values >= 5)[0]])
    over_10 = sum(coverage_bases[np.where(coverage_values >= 10)[0]])
    over_30 = sum(coverage_bases[np.where(coverage_values >= 30)[0]])
    over_50 = sum(coverage_bases[np.where(coverage_values >= 50)[0]])
    over_100 = sum(coverage_bases[np.where(coverage_values >= 100)[0]])
    over_200 = sum(coverage_bases[np.where(coverage_values >= 200)[0]])
    # read the array to calculate percentile
    p10,p25,p50,p75,p90 = compute_percentle_from_two_arrays(coverage_values, coverage_bases, total_bases) 
    piechart_value = { 
    "0X": zero_cov,
    "[1,5X)": over_1_less_5,
...
def calculate_depth(bam_file, coverage_file, annotation=None, region=None, bed=None, gene=None, quality=0):
    ...
    depths = {} # a dict to store {coverage: how many bases is at this coverage}
    total_bases = 0
    if region or gene:
        tmp_region=None
        if gene:
        ...
        for pileupcolumn in bam.pileup(chrom, start, end, truncate=True,min_mapping_quality = quality):
            if pileupcolumn.nsegments in depths.keys():
                depths[pileupcolumn.nsegments] += 1 # count coverage number
            else:
                depths[pileupcolumn.nsegments] = 1
    ...
    bam.close()
    depths[0]=total_bases - sum(depths.values())  # add zero coverage base number to the dict
    depths_sorted=dict(sorted(depths.items())) # sort depth from low to high
#    return depths, total_bases
    piechart_value, coverage_report = compute_coverage_stats(depths_sorted, total_bases)
    return piechart_value, coverage_report
...
def main():
    if len(sys.argv)<2:
        ...
        piechart_value,stats = calculate_depth(bam_file=input_bam, coverage_file=coverage_file, annotation=annotation, region=region, bed=bed, gene=gene)
...
#    
        ...
        if draw :
            ...
            if bed:
                output_pdf_dir=f"{output_prefix}/{output_prefix}.track.files"
                bed_regions = BedTool(bed)
                for interval in bed_regions:
