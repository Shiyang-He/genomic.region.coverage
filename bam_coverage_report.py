...
in reality, this function is not good for large region compute, need to refine: best to store these values in a dict when generate them, do not use depth in a array, which comsume a lot of memory'''
    ...
    coverage_report = {
        ...
        "Max Depth": max_depth,
        ">=1X": str(over_1) + "\t" + str(round(over_1/total_bases,3)),
        ">=5X": str(over_5) + "\t" + str(round(over_5/total_bases,3)),
        ">=10X": str(over_10) + "\t" + str(round(over_10/total_bases,3)),
        ">=30X": str(over_30) + "\t" + str(round(over_30/total_bases,3)),
        ">=50X": str(over_50) + "\t" + str(round(over_50/total_bases,3)),
        ">=100X": str(over_100) + "\t" + str(round(over_100/total_bases,3)),
        ">=200X": str(over_200) + "\t" + str(round(over_200/total_bases,3)) ,
        "0X": str(zero_cov) + "\t" + str(round(zero_cov/total_bases,3)),
        "[1,5X)": str(over_1_less_5) + "\t" + str(round(over_1_less_5/total_bases,3)),
        "[5,10X)": str(over_5_less_10) + "\t" + str(round(over_5_less_10/total_bases,3)),
        "[10,30X)": str(over_10_less_30) + "\t" + str(round(over_10_less_30/total_bases,3)),
        "[30,50X)": str(over_30_less_50) + "\t" + str(round(over_30_less_50/total_bases,3)),
        "[50,100X)": str(over_50_less_100) + "\t" + str(round(over_50_less_100/total_bases,3)),
        "[100,200X)": str(over_100_less_200) + "\t" + str(round(over_100_less_200/total_bases,3)),
        "10th Percentile": p10,
        "25th Percentile": p25,
        "50th Percentile": p50,
        "75th Percentile": p75,
